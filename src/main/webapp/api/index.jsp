...
<div class="home-index main-padding-bottom">
	<div class="pullRefresh">
		...
		<div class="pageView">
			<div class="slide">
				<ul>
					...
					<li ad_type="${g.getAd_type()}" ad_content="${g.getAd_content()}" pic="${g.getPic()}"></li>
					</c:forEach>
				</ul>
			</div>
			<div class="pager"></div>
		...
		<div class="cate">
			...
			<a href="/goods?category_id=${g.getId()}&title=${g.getName()}"><div url="${g.getPic()}"></div><span>${g.getName()}</span></a>
			</c:forEach>
		</div>
		<%}%>
		...
		<ul class="list goods-item">
			...
			<li>
				<a href="/goods/detail?id=${g.getId()}">
					<div class="pic" url="${g.getPic()}"></div>
					<div class="title"><div>${g.getName()}</div><font><c:if test="${g.getPurchase_price()>0}">正在秒杀中</c:if></font><span><strong>￥<fmt:formatNumber type="number" pattern="0.00" value="${g.getPrice()}" /></strong><s>￥<fmt:formatNumber type="number" pattern="0.00" value="${g.getMarket_price()}" /></s></span></div>
				</a>
			</li>
			</c:forEach>
		</ul>
...
<div class="footer">
	<a class="ico1 this" href="${pageContext.request.contextPath}"></a>
	<a class="ico2" href="${pageContext.request.contextPath}/category"></a>
	<a class="ico3" href="${pageContext.request.contextPath}/article"></a>
	<a class="ico4 badge" href="${pageContext.request.contextPath}/cart"></a>
	<a class="ico5" href="${pageContext.request.contextPath}/member"></a>
</div>
