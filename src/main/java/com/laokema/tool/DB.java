...
public class DB {
	...
	private HttpServletRequest pagination = null;
	...
	//设置分页
	public DB pagination(HttpServletRequest pagination) {
		return pagination(pagination, "page");
	}
	public DB pagination(HttpServletRequest pagination, String paginationMark) {
	...
	public <T> List<T> select(Class<T> clazz) {
		...
		try {
			...
			if (this.cached != 0) {
				...
				if (r != null) return r;
			} else if (this.pagination != null && this.pagesize != 1) {
				_setPagination();
			}
			if (conn == null) DB.init();
			ps = conn.prepareStatement(sql);
	...
	private void _setPagination() {
		...
		try {
			...
			Pagination p = new Pagination(this.pagination, records).isCn();
			this.pagination.setAttribute(this.paginationMark, p);
		} catch (Exception e) {
	...
	private <T> List<T> _cacheSql(String sql, Class<T> clazz) {
		if (rootPath.length() == 0) {
			String path = Objects.requireNonNull(DB.class.getResource("../../../../../WEB-INF")).getPath();
